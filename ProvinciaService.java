...
public class ProvinciaService {
    ...
    public String excluir(Long codigoProvincia) throws CPJException{
        try {
            if(codigoProvincia == null) {
                return "Nenhuma provincia informada. Selecione a provincia a ser excluída.";
            }
            return provinciaDAO.excluir(codigoProvincia)? "Provincia excluida com sucesso.":"Código inexistente. Não foi possível excluír a provincia informada.";
        } catch (ClassNotFoundException ex) {
            throw new CPJException("Driver do MySQL não encontrado.");
        } catch (SQLException ex) {
            throw new CPJException("Erro: " + ex.getMessage());
    ...
    public List<Provincia> filtrar(Long linhaInicial, Long totalDeLinhas) throws CPJException{
        try {
            return provinciaDAO.filtrar(linhaInicial, totalDeLinhas);
        } catch (ClassNotFoundException ex) {
            throw new CPJException("Driver do MySQL não encontrado.");
        } catch (SQLException ex) {
            throw new CPJException("Erro: " + ex.getMessage());
